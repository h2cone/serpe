...
func newConversation(req *models.Request, tools []models.Tool) (*conversation, error) {
	if req == nil {
		return nil, &models.Error{Kind: models.ErrorInvalidRequest, Operation: "agent", Message: "request is nil"}
	}
	if len(req.Tools) > 0 {
		return nil, &models.Error{Kind: models.ErrorInvalidRequest, Operation: "agent", Message: "request tools must be empty; register tools on the Runner"}
	}
	if req.Generation.CandidateCount.Set && req.Generation.CandidateCount.Value != 1 {
		return nil, &models.Error{Kind: models.ErrorInvalidRequest, Operation: "agent", Message: "candidate count must be unset or 1"}
	}
	if len(req.Messages) == 0 {
		return nil, &models.Error{Kind: models.ErrorInvalidRequest, Operation: "agent", Message: "at least one message is required"}
	}
	if last := req.Messages[len(req.Messages)-1]; last.Role != models.RoleUser {
		return nil, &models.Error{Kind: models.ErrorInvalidRequest, Operation: "agent", Message: "last message must be a user message"}
	}
	snapshot := req.Clone()
