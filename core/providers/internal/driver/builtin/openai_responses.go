...
func NewOpenAIResponses(config shared.Config) *Provider {
	return NewProvider(config, Adapter{
		...
		Decode: func(raw []byte, requestID, _ string, config shared.Config, limits models.StreamLimits) (*models.Response, error) {
			return responses.DecodeResponseJSONWithLimits(raw, requestID, config.Limits.MaxProviderStateBytes, shared.ToolCallLimitsFromStream(limits))
		},
		NewSource: func(reader *sse.Reader, requestID, modelID string, config shared.Config, limits models.StreamLimits) models.EventSource {
			return responses.NewSSEStreamSource(reader, requestID, modelID, config.Policy.IgnoreUnknownEvent, config.Limits.MaxProviderStateBytes, shared.ToolCallLimitsFromStream(limits))
		},
	})
}
