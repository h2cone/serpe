...
func encodeRequest(modelID string, req *models.Request, stream bool) ([]byte, error) {
	wire := chatRequest{Model: modelID, Stream: stream}
	if stream {
	...
	for _, tool := range req.Tools {
		function := chatFunction{Name: tool.Name, Description: tool.Description, Parameters: append(json.RawMessage(nil), tool.Parameters...)}
		if tool.Strict.Set {
			value := tool.Strict.Value
	...
	if req.Generation.MaxOutputTokens.Set {
		...
		wire.MaxCompletionTokens = &value
	}
	...
	wire.Stop = append([]string(nil), req.Generation.Stop...)
	if req.Generation.Seed.Set {
		value := req.Generation.Seed.Value
		wire.Seed = &value
	}
	if req.Generation.CandidateCount.Set {
		value := req.Generation.CandidateCount.Value
		wire.N = &value
	}
...
func encodeResponseFormat(format models.ResponseFormat) (json.RawMessage, error) {
	...
	case models.ResponseFormatJSONSchema:
		...
		value := schema{Name: format.Name, Description: format.Description, Schema: format.Schema}
		if format.Strict.Set {
			strict := format.Strict.Value
			value.Strict = &strict
		}
		return marshalJSON(struct {
			Type       string `json:"type"`
