...
func (b *Bridge) Do(req *http.Request) (*http.Response, error) {
	...
	requestBody, replaceRequestBody := requestBodyFrom(ctx)
	if caller, ok := callerContext(ctx); ok {
		ctx = caller
		if capture != nil {
		...
		// Preserve the original caller marker so nested retries still neutralize.
		ctx = withCallerContext(ctx)
	...
	for _, secret := range b.cfg.PlaceholderAuth {
		if secret == "" {
			continue
		}
