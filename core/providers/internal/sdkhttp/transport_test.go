...
func TestPrepareStreamCallKeepsInternalCancellation(t *testing.T) {
	...
	if capture == nil || captureFrom(ctx) != capture {
		t.Fatal("stream capture was not attached")
	}
	cancel()
	caller, ok := callerContext(ctx)
	if !ok {
		t.Fatal("caller marker is missing")
	}
	if !errors.Is(caller.Err(), context.Canceled) {
		t.Fatalf("caller marker error = %v, want context canceled", caller.Err())
	}
}
