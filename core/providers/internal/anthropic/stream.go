...
func newMessageSource(reader *sse.Reader, requestID, fallbackModel string, ignoreUnknown bool, stateLimit int64) *messageSource {
...
func (s *messageSource) consume(event streamEventWire, eventID string) error {
	...
	case "message_stop":
		...
		if s.hasState {
			blockIndexes := make([]int, 0, len(s.stateBlocks))
			for index := range s.stateBlocks {
