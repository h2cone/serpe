...
func (m *model) Complete(ctx context.Context, req *models.Request) (*models.Response, error) {
	...
	response, err := m.provider.http.Do(ctx, "generate", "/v1/messages", nil, payload, false, req.RequestID, extra)
	if err != nil {
		return nil, err
	}
	var wire messageWire
	if err := httpx.ReadJSON(response, m.provider.config.Limits.MaxResponseBytes, "anthropic", "generate", &wire); err != nil {
		return nil, err
	}
...
func (m *model) Stream(ctx context.Context, req *models.Request) (models.Stream, error) {
	...
	extra := make(http.Header)
	extra.Set("Anthropic-Version", "2023-06-01")
	response, err := m.provider.http.Do(ctx, "stream", "/v1/messages", nil, payload, true, req.RequestID, extra)
	if err != nil {
		return nil, err
	}
	source := newMessageSource(sse.NewReader(response.Body, m.provider.config.Limits.MaxSSEEventBytes), httpx.RequestID(response.Header), m.modelID, m.provider.config.Policy.IgnoreUnknownEvent, m.provider.config.Limits.MaxProviderStateBytes)
...
func (m *model) encode(req *models.Request, stream bool) ([]byte, error) {
	if err := models.ValidateCapabilities(req, capabilities, "anthropic"); err != nil {
		return nil, err
	}
