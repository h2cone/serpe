...
func TestRunSingleToolThenAnswer(t *testing.T) {
	...
	tool := newStubTool("echo", func(_ context.Context, args json.RawMessage) (runtime.ToolOutput, error) {
		return runtime.TextResult("echo:" + string(args)), nil
	})
	r, err := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{tool}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunMultiToolsOrder(t *testing.T) {
	...
	var order []string
	var mu sync.Mutex
	makeTool := func(name string) runtime.Tool {
		return newStubTool(name, func(_ context.Context, _ json.RawMessage) (runtime.ToolOutput, error) {
			mu.Lock()
			order = append(order, name)
			mu.Unlock()
			return runtime.TextResult(name), nil
		})
	}
	model := &scriptedModel{responses: []*models.Response{
		toolCallResponse(
	...
	r, err := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{makeTool("a"), makeTool("b")}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecoverableToolErrorResult(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (runtime.ToolOutput, error) {
		return runtime.ErrorResult("bad args"), nil
	})
	r, _ := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{tool}})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestFatalToolError(t *testing.T) {
	...
	r, _ := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{tool}})
	result, err := r.Run(context.Background(), userReq("go"))
	if err == nil || !errors.Is(err, runtime.ErrToolExecution) {
		t.Fatalf("err=%v", err)
	}
	if result == nil || result.StopReason != runtime.StopFailed {
...
func TestFatalToolErrorPreservesEarlierBatchResults(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (runtime.ToolOutput, error) {
		if atomic.AddInt32(&executions, 1) == 1 {
			return runtime.TextResult("first completed"), nil
		}
		return runtime.ToolOutput{}, errors.New("second failed")
	})
	r, _ := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{tool}})
	result, err := r.Run(context.Background(), userReq("go"))
	if err == nil || !errors.Is(err, runtime.ErrToolExecution) {
...
func TestDuplicateCallID(t *testing.T) {
	...
	r, _ := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{tool}})
	result, err := r.Run(context.Background(), userReq("go"))
	if err == nil || !errors.Is(err, runtime.ErrInvalidModelResponse) {
		t.Fatalf("err=%v", err)
	}
	if result.StopReason != runtime.StopFailed {
		t.Fatalf("stop=%s", result.StopReason)
	}
}
...
func TestRequestValidation(t *testing.T) {
	...
	r, _ := runtime.NewRunner(runtime.Config{Model: model})
...
func TestMaxModelTurns(t *testing.T) {
	...
	r, _ := runtime.NewRunner(runtime.Config{
		Model:  model,
	...
	if result.StopReason != runtime.StopMaxModelTurns {
		t.Fatalf("stop=%s", result.StopReason)
	}
...
func TestMaxToolCalls(t *testing.T) {
	...
	r, _ := runtime.NewRunner(runtime.Config{
		Model:  model,
		Tools:  []runtime.Tool{tool},
		Limits: runtime.Limits{MaxToolCalls: 1},
	})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
	if result.StopReason != runtime.StopMaxToolCalls {
		t.Fatalf("stop=%s", result.StopReason)
	}
	if atomic.LoadInt32(&execs) != 0 {
...
func TestMaxObservedTokens(t *testing.T) {
	...
	r, _ := runtime.NewRunner(runtime.Config{
		Model:  model,
		Tools:  []runtime.Tool{tool},
		Limits: runtime.Limits{MaxObservedTokens: 50},
	})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
	if result.StopReason != runtime.StopMaxObservedTokens {
...
func TestMaxObservedTokensFinalAnswerStillCompletes(t *testing.T) {
	...
	r, _ := runtime.NewRunner(runtime.Config{Model: model, Limits: runtime.Limits{MaxObservedTokens: 50}})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStallDetection(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (runtime.ToolOutput, error) {
		return runtime.TextResult("same"), nil
	})
	r, _ := runtime.NewRunner(runtime.Config{
		Model:  model,
		Tools:  []runtime.Tool{tool},
		Limits: runtime.Limits{MaxIdenticalSteps: 3},
	})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
	if result.StopReason != runtime.StopStalled {
		t.Fatalf("stop=%s", result.StopReason)
	}
}
...
func TestStallResetsOnChange(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (runtime.ToolOutput, error) {
		...
		return runtime.TextResult(string(rune('a' + v - 1))), nil
	})
	r, _ := runtime.NewRunner(runtime.Config{
		Model:  model,
		Tools:  []runtime.Tool{tool},
		Limits: runtime.Limits{MaxIdenticalSteps: 2},
	})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
...
func TestModelTerminalClassification(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			r, _ := runtime.NewRunner(runtime.Config{Model: model})
			result, err := r.Run(context.Background(), userReq("go"))
			if err == nil || !errors.Is(err, runtime.ErrModelResponse) {
				t.Fatalf("err=%v", err)
			}
			...
			if tt.cancelled {
				if !errors.Is(err, context.Canceled) || result.StopReason != runtime.StopCancelled {
					t.Fatalf("cancelled err=%v stop=%s", err, result.StopReason)
				}
			} else if result.StopReason != runtime.StopFailed {
				t.Fatalf("stop=%s", result.StopReason)
			}
		})
	}
...
func TestNonSuccessToolResponseDoesNotExecute(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (runtime.ToolOutput, error) {
		...
		return runtime.TextResult("unexpected"), nil
	})
	model := &scriptedModel{responses: []*models.Response{response}}
	r, _ := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{tool}})
	result, err := r.Run(context.Background(), userReq("go"))
...
func TestToolChoiceResponseIsEnforcedBeforeExecution(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			model := &scriptedModel{responses: []*models.Response{tt.response}}
			r, _ := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{makeTool("a"), makeTool("b")}})
			req := userReq("go")
...
func TestObservedTokenAccountingRejectsNegativeAndSaturatesOverflow(t *testing.T) {
	...
	t.Run("overflow", func(t *testing.T) {
		...
		r, _ := runtime.NewRunner(runtime.Config{
			Model: model, Tools: []runtime.Tool{tool}, Limits: runtime.Limits{MaxObservedTokens: math.MaxInt64},
		})
		result, err := r.Run(context.Background(), userReq("go"))
		if err != nil {
			t.Fatal(err)
		}
		if result.StopReason != runtime.StopMaxObservedTokens || result.ObservedTotalTokens != math.MaxInt64 {
			t.Fatalf("stop=%s observed=%d", result.StopReason, result.ObservedTotalTokens)
		}
...
func TestConcurrentRuns(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (runtime.ToolOutput, error) {
		...
		return runtime.TextResult("x"), nil
	})
	r, _ := runtime.NewRunner(runtime.Config{Model: model, Tools: []runtime.Tool{tool}})
