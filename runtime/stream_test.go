...
func TestStreamEventOrder(t *testing.T) {
	...
	want := []runtime.EventKind{
		runtime.EventRunStart,
		runtime.EventModelStart,
		runtime.EventModel,
		runtime.EventModel,
		runtime.EventModel,
		runtime.EventModel,
		runtime.EventModelEnd,
		runtime.EventToolStart,
		runtime.EventToolEnd,
		runtime.EventModelStart,
		runtime.EventModel,
		runtime.EventModel,
		runtime.EventModel,
...
func TestRunStreamEquivalence(t *testing.T) {
	...
	newRunner := func() *runtime.Runner {
		...
		r, err := runtime.NewRunner(runtime.Config{Model: m, Tools: []runtime.Tool{newStubTool("f", nil)}})
		if err != nil {
			t.Fatal(err)
		}
		return r
...
func TestNilModelStreamFailsWithoutPanic(t *testing.T) {
	t.Parallel()
	r, _ := runtime.NewRunner(runtime.Config{Model: nilStreamModel{}})
...
func TestCloseDuringModelStreamAcquisitionClosesReturnedStream(t *testing.T) {
	...
	if !stream.Next() || stream.Event().Kind != runtime.EventRunStart {
...
func TestCancelContext(t *testing.T) {
	...
	started := make(chan struct{})
	tool := newStubTool("f", func(ctx context.Context, _ json.RawMessage) (runtime.ToolOutput, error) {
		close(started)
		<-ctx.Done()
		return runtime.ToolOutput{}, ctx.Err()
	})
	model := &scriptedModel{responses: []*models.Response{
	...
	result := stream.Result()
	if result.StopReason != runtime.StopCancelled {
