...
func TestRunSingleToolThenAnswer(t *testing.T) {
	...
	tool := newStubTool("echo", func(_ context.Context, args json.RawMessage) (tools.Output, error) {
		return tools.Text("echo:" + string(args)), nil
	})
	r, err := loops.New(loops.Config{Model: model, Tools: mustTools(t, tool)})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunMultiToolsOrder(t *testing.T) {
	...
	r, err := loops.New(loops.Config{Model: model, Tools: mustTools(t, makeTool("a"), makeTool("b"))})
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnknownToolRecoverable(t *testing.T) {
	...
	r, err := loops.New(loops.Config{Model: model, Tools: mustTools(t)})
	if err != nil {
		t.Fatal(err)
	}
	result, err := r.Run(context.Background(), userReq("go"))
...
func TestRecoverableToolErrorResult(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (tools.Output, error) {
		return tools.Error("bad args"), nil
	})
	r, _ := loops.New(loops.Config{Model: model, Tools: mustTools(t, tool)})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestFatalToolErrorPreservesEarlierBatchResults(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (tools.Output, error) {
		if atomic.AddInt32(&executions, 1) == 1 {
			return tools.Text("first completed"), nil
		}
		return tools.Output{}, errors.New("second failed")
	})
	r, _ := loops.New(loops.Config{Model: model, Tools: mustTools(t, tool)})
	result, err := r.Run(context.Background(), userReq("go"))
	if err == nil || !errors.Is(err, loops.ErrToolExecution) {
...
func TestDuplicateCallID(t *testing.T) {
	...
	if result.StopReason != loops.StopFailed {
		t.Fatalf("stop=%s", result.StopReason)
	}
}

func TestHistoricalCallIDCannotBeReused(t *testing.T) {
	t.Parallel()
	model := &scriptedModel{responses: []*models.Response{
		toolCallResponse(models.ToolCall{ID: "prior", Name: "f", Arguments: json.RawMessage(`{}`)}),
	}}
	var executions atomic.Int32
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (tools.Output, error) {
		executions.Add(1)
		return tools.Text("unexpected"), nil
	})
	runner, err := loops.New(loops.Config{Model: model, Tools: mustTools(t, tool)})
	if err != nil {
		t.Fatal(err)
	}
	req := &models.Request{Messages: []models.Message{
		models.NewUserMessage(models.Text("first")),
		models.NewAssistantMessage(models.ToolCallContent("prior", "f", json.RawMessage(`{}`))),
		models.NewUserMessage(models.ToolResultContent("prior", "f", false, models.Text("done"))),
		models.NewUserMessage(models.Text("continue")),
	}}
	_, err = runner.Run(context.Background(), req)
	if err == nil || !errors.Is(err, loops.ErrInvalidModelResponse) {
		t.Fatalf("err = %v, want ErrInvalidModelResponse", err)
	}
...
func TestDuplicateHistoricalCallIDRejectedBeforeModel(t *testing.T) {
	t.Parallel()
	model := &scriptedModel{responses: []*models.Response{textResponse("unexpected")}}
	runner, err := loops.New(loops.Config{Model: model, Tools: mustTools(t, newStubTool("f", nil))})
	if err != nil {
		t.Fatal(err)
	}
	req := &models.Request{Messages: []models.Message{
		models.NewUserMessage(models.Text("first")),
		models.NewAssistantMessage(models.ToolCallContent("same", "f", json.RawMessage(`{}`))),
		models.NewUserMessage(models.ToolResultContent("same", "f", false, models.Text("one"))),
		models.NewAssistantMessage(models.ToolCallContent("same", "f", json.RawMessage(`{}`))),
		models.NewUserMessage(models.ToolResultContent("same", "f", false, models.Text("two"))),
		models.NewUserMessage(models.Text("continue")),
	}}
	_, err = runner.Run(context.Background(), req)
	if err == nil || !errors.Is(err, loops.ErrInvalidModelResponse) {
...
func TestRequestValidation(t *testing.T) {
	...
	r, _ := loops.New(loops.Config{Model: model})
...
func TestMaxModelTurns(t *testing.T) {
	...
	if result.StopReason != loops.StopMaxModelTurns {
		t.Fatalf("stop=%s", result.StopReason)
	}
...
func TestMaxObservedTokens(t *testing.T) {
	...
	r, _ := loops.New(loops.Config{
		Model:  model,
		Tools:  mustTools(t, tool),
		Limits: loops.Limits{MaxObservedTokens: 50},
	})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
	if result.StopReason != loops.StopMaxObservedTokens {
...
func TestMaxObservedTokensFinalAnswerStillCompletes(t *testing.T) {
	...
	r, _ := loops.New(loops.Config{Model: model, Limits: loops.Limits{MaxObservedTokens: 50}})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStallDetection(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (tools.Output, error) {
		return tools.Text("same"), nil
	})
	r, _ := loops.New(loops.Config{
		Model:  model,
		Tools:  mustTools(t, tool),
		Limits: loops.Limits{MaxIdenticalSteps: 3},
	})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
	}
	if result.StopReason != loops.StopStalled {
		t.Fatalf("stop=%s", result.StopReason)
	}
}
...
func TestStallResetsOnChange(t *testing.T) {
	...
	tool := newStubTool("f", func(_ context.Context, _ json.RawMessage) (tools.Output, error) {
		...
		return tools.Text(string(rune('a' + v - 1))), nil
	})
	r, _ := loops.New(loops.Config{
		Model:  model,
		Tools:  mustTools(t, tool),
		Limits: loops.Limits{MaxIdenticalSteps: 2},
	})
	result, err := r.Run(context.Background(), userReq("go"))
	if err != nil {
		t.Fatal(err)
...
func TestModelTerminalClassification(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			r, _ := loops.New(loops.Config{Model: model})
			result, err := r.Run(context.Background(), userReq("go"))
			if err == nil || !errors.Is(err, loops.ErrModelResponse) {
				t.Fatalf("err=%v", err)
			}
			...
			if tt.cancelled {
				if !errors.Is(err, context.Canceled) || result.StopReason != loops.StopCancelled {
					t.Fatalf("cancelled err=%v stop=%s", err, result.StopReason)
				}
			} else if result.StopReason != loops.StopFailed {
				t.Fatalf("stop=%s", result.StopReason)
			}
		})
	}
...
func TestToolChoiceResponseIsEnforcedBeforeExecution(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			model := &scriptedModel{responses: []*models.Response{tt.response}}
			r, _ := loops.New(loops.Config{Model: model, Tools: mustTools(t, makeTool("a"), makeTool("b"))})
			req := userReq("go")
...
func TestObservedTokenAccountingRejectsNegativeAndSaturatesOverflow(t *testing.T) {
	...
	t.Run("overflow", func(t *testing.T) {
		...
		r, _ := loops.New(loops.Config{
			Model: model, Tools: mustTools(t, tool), Limits: loops.Limits{MaxObservedTokens: math.MaxInt64},
		})
		result, err := r.Run(context.Background(), userReq("go"))
		if err != nil {
			t.Fatal(err)
		}
		if result.StopReason != loops.StopMaxObservedTokens || result.ObservedTotalTokens != math.MaxInt64 {
			t.Fatalf("stop=%s observed=%d", result.StopReason, result.ObservedTotalTokens)
		}
