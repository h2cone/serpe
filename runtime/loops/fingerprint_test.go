...
func TestStepFingerprintIgnoresCallID(t *testing.T) {
	...
	results := []tools.Output{tools.Text("ok")}
...
func TestStepFingerprintResultChange(t *testing.T) {
	...
	fp1, _ := stepFingerprint(calls, []tools.Output{tools.Text("a")})
	fp2, _ := stepFingerprint(calls, []tools.Output{tools.Text("b")})
	if fp1 == fp2 {
...
func TestStepFingerprintMultiCallOrder(t *testing.T) {
	...
	results := []tools.Output{tools.Text("x"), tools.Text("x")}
