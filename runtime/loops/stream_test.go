...
func TestStreamEventOrder(t *testing.T) {
	...
	want := []loops.EventKind{
		loops.EventRunStart,
		loops.EventModelStart,
		loops.EventModel,
		loops.EventModel,
		loops.EventModel,
		loops.EventModel,
		loops.EventModelEnd,
		loops.EventToolStart,
		loops.EventToolEnd,
		loops.EventModelStart,
		loops.EventModel,
		loops.EventModel,
...
func TestRunStreamEquivalence(t *testing.T) {
	...
	newRunner := func() *loops.Runner {
		...
		r, err := loops.New(loops.Config{Model: m, Tools: mustTools(t, newStubTool("f", nil))})
		if err != nil {
			t.Fatal(err)
		}
		return r
...
func TestNilModelStreamFailsWithoutPanic(t *testing.T) {
	t.Parallel()
	r, _ := loops.New(loops.Config{Model: nilStreamModel{}})
...
func TestCloseDuringModelStreamAcquisitionClosesReturnedStream(t *testing.T) {
	...
	if !stream.Next() || stream.Event().Kind != loops.EventRunStart {
...
func TestCancelContext(t *testing.T) {
	...
	started := make(chan struct{})
	tool := newStubTool("f", func(ctx context.Context, _ json.RawMessage) (tools.Output, error) {
		close(started)
		<-ctx.Done()
	...
	result := stream.Result()
	if result.StopReason != loops.StopCancelled {
